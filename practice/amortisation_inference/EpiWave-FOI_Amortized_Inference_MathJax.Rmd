...
**Training library (simulation):** 1. Sample $\Theta^{(i)} \sim \Pi_\Theta$. 2. Solve ODE: $(x^{(i)},z^{(i)})=\mathcal{S}(\Theta^{(i)},t)$. 3. Map to data: $\hat y^{(i)}=g(x^{(i)},z^{(i)})$. 4. Sample observations: $y^{(i)}\sim \mathcal{T}_y(\hat y^{(i)})$. 5. Train $\text{NN}_{\text{amort}}$ on pairs $(y^{(i)},\Theta^{(i)})$ to learn $p(\Theta\mid y)$ (e.g., via distributional outputs or normalizing flows).
