...
| Item |   |   | Notes |
|----|----|----|----|
| Lock Δt strategy (fixed vs adaptive) and seasonal regimes |  |  | Start with fixed Δt; pilot adaptive in high‑variance zones |
| Finalize `iterate_state` signature + unit tests |  |  | Ensure bounds checks keep states ∈ [0,1] |
| Prior calibration for `log(m)` model |  |  | β truncated ≥0; α centered at 0; sensitivity to σ |
| Define covariates `X` and data joins (Vector Atlas, MosquitoDB, NMCP) |  |  | Track provenance and temporal alignment |
| Uncertainty products (ensembles → FOI maps) |  |  | Summaries: mean, median, 50/90% CrI |
| Policy dashboard spec |  |  | Include before/after intervention deltas |
